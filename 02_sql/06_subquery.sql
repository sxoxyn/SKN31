...
- 서브쿼리 조회결과 행수에 따른 구분
    - '단일행' 서브쿼리 - 서브쿼리의 '조회결과 행이 한행'인 것.
    - '다중행' 서브쿼리 - 서브쿼리의 '조회결과 행이 여러행'인 것.
- 동작 방식에 따른 구분
    - '비상관' 서브쿼리 - 서브쿼리에 메인쿼리의 컬럼이 사용되지 않는다.
                '메인쿼리에 사용할 값을 서브쿼리가 제공'하는 역할을 한다.
    - '상관' 서브쿼리 - '서브쿼리에서 메인쿼리의 컬럼을 사용'한다. 
				메인쿼리가 '먼저 수행되어 읽혀진 데이터'를 '서브쿼리에서 조건이 맞는지 확인'하고자 할때 주로 사용한다.

- '서브쿼리'는 반드시 '( ) 로 묶어줘야' 한다.
************************************************************************** */
...
select * from emp
where (job_id != 'ST_CLERK' or job_id is null) -- null은 모르는 값이니까 관련 내용 적어주지 않으면 안 나옴. is null로 job_id가 null인 정보도 포함해서 나오도록 해야 함.
