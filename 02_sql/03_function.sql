...
다중행 함수: 
	- 여러행을 묶어서 한번에 처리하는 함수 => '집계(합계, 평균 등)'함수, 그룹함수라고 한다.
	- 다중행은 where절에는 사용할 수 없다. (sub query 이용) 
    - select절과 having 절에서 쓰임
* ***********************************************/
...
*************************************************************************************************************** */
use testdb;
-- 함수에 넣어주는 파라미터 변수는 컬럼이 갖고 있는 많은 값을 넣어줌. (단일행이기 때문에 한 줄씩나온 결과를 묶어서 줌)
...
select round(12345.12345,  2); # 자리 2 이하에서 반올림 - 특정 테이블에서 조회하려고 하는 것이 아니기 때문에 from절 사용 안 함
...
select subdate(curdate(), interval 10 month), -- 이전 날짜(10개월 전)
       curdate(),
       adddate(curdate(), interval 10 month) -- 이후 날짜(10개월 후)
;
select adddate(now(), interval 3 week) "3주후",
	   adddate(now(), interval 10 hour) "10시간 후",
       subdate(now(), interval -10 hour) "10시간 전";

-- 날짜/시간의 차이 계산
select datediff(curdate(), '2024-10-10'); -- 오늘 - 2024-10-10 며칠 지났는지
