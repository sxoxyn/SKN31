...
# 대화 내역을 session_state에서 조회, 없으면 등록(초기화)
## session_state: directionary 형태
if "chat_history" not in st.session_state: # key 조회
    st.session_state['chat_history'] = [] # 없으면 chat_history라는 이름으로 빈 list 생성
    # 개별 메시지를 dictionary로 저장 {"role":"user/ai", "content":"메시지"} - OpenAI 메시지 형식

prompt = st.chat_input("User : ")
if prompt:
    # prompt와 AI 응답 메시지를 chat_history에 추가
    # prompt 추가
